...
train_ds = ICIFAR100(data_path="/ibex/ai/home/jansonrp/dacgzsl/data", train=True, download=True)
test_ds = ICIFAR100(data_path="/ibex/ai/home/jansonrp/dacgzsl/data", train=False, download=True)
...
per_task_accuracy = []
for task_id,test_ds in enumerate(scenario_test):
    test_loader = DataLoader(test_ds, batch_size=512)
    correct , total = 0 , 0
    for (img_batch,label,t) in test_loader:
        img_batch = img_batch.cuda()
        ...
        correct += (predictions.cpu() == label.cpu()).sum()
        total += label.shape[0]
    print(f"Per task Accuracy at {task_id} {correct/total}") 
print(f"average per task accuracy {round(np.mean(np.array(per_task_accuracy))* 100,2)} ")

average = [0.8679999709129333,0.8539999723434448,]