...
cifar_test_loader = torch.utils.data.DataLoader(cifar10_test_ds, batch_size=1024, shuffle=False, num_workers=4)
...
not_mnist_test_loader = torch.utils.data.DataLoader(not_mnist_test_ds, batch_size=1024, shuffle=False, num_workers=4)
...
mnist_test_loader = torch.utils.data.DataLoader(mnist_test_ds, batch_size=1024, shuffle=False, num_workers=4)
...
svhn_test_loader = torch.utils.data.DataLoader(svhn_test_ds, batch_size=1024, shuffle=False, num_workers=4)
...
fashion_mnist_test_loader = torch.utils.data.DataLoader(fashionmnist_test_ds, batch_size=1024, shuffle=False, num_workers=4)
...
per_task_accuracy = []
for (img_batch,label) in cifar_test_loader:
    img_batch = img_batch.cuda()
    with torch.no_grad():
        out = F.normalize(vit_b_16.forward_features(img_batch)[:,0].detach()).cpu().numpy()
    predictions = []
    ...
    total += label.shape[0]
print(f"Accuracy of cifar {correct/total}")
per_task_accuracy.append(correct/total)

for (img_batch,label,tt,td) in not_mnist_test_loader:
    img_batch = img_batch.cuda()
    with torch.no_grad():
        out = F.normalize(vit_b_16.forward_features(img_batch)[:,0].detach()).cpu().numpy()
    predictions = []
    ...
    correct += (predictions.cpu() == label.cpu()).sum()
    total += label.shape[0]
print(f"Accuracy at not mnist {correct/total}")
per_task_accuracy.append(correct/total)

# task 2 

for (img_batch,label) in mnist_test_loader:
    img_batch = img_batch.cuda()
    with torch.no_grad():
        out = F.normalize(vit_b_16.forward_features(img_batch)[:,0].detach()).cpu().numpy()
    predictions = []
    ...
    correct += (predictions.cpu() == label.cpu()).sum()
    total += label.shape[0]
print(f"Accuracy at mnist {correct/total}")
per_task_accuracy.append(correct/total)

for (img_batch,label) in svhn_test_loader:
    img_batch = img_batch.cuda()
    with torch.no_grad():
        out = F.normalize(vit_b_16.forward_features(img_batch)[:,0].detach()).cpu().numpy()
    predictions = []
    ...
    correct += (predictions.cpu() == label.cpu()).sum()
    total += label.shape[0]
print(f"Accuracy at svhn {correct/total}")
per_task_accuracy.append(correct/total)

for (img_batch,label) in fashion_mnist_test_loader:
    img_batch = img_batch.cuda()
    with torch.no_grad():
        out = F.normalize(vit_b_16.forward_features(img_batch)[:,0].detach()).cpu().numpy()
    predictions = []
    ...
    correct += (predictions.cpu() == label.cpu()).sum()
    total += label.shape[0]
print(f"Accuracy at fashion {correct/total}")
per_task_accuracy.append(correct/total)

print(f"Average per_task accuracy {np.mean(per_task_accuracy)}")